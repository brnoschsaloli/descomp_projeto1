	...
	architecture arquitetura of CPU is
	  -- Instanciando os componentes:
		signal Saida_MUX_ULA : std_logic_vector (larguraDados_ROM-6 downto 0);
		signal Saida_MUX_JMP : std_logic_vector (larguraDados_ROM-5 downto 0);
		signal REG1_ULA_A : std_logic_vector (larguraDados_ROM-6 downto 0);
		signal REG_FLAG: std_logic;
		signal Saida_ULA : std_logic_vector (larguraDados_ROM-6 downto 0);
	...
	MUX_ULA :  entity work.muxGenerico2x1  generic map (larguraDados => larguraEnderecos_ROM-1)
			  port map( entradaA_MUX => Data_IN,
						  entradaB_MUX =>  Instruction_IN(larguraDados_ROM-6 downto 0),
	...
	MUX_JMP :  entity work.muxGenerico4x1  generic map (larguraDados => larguraEnderecos_ROM)
			  port map( entrada0_MUX => proxPC,
						  entrada1_MUX =>  Instruction_IN(larguraDados_ROM-5 downto 0),
	...
	REGA : entity work.registradorGenerico   generic map (larguraDados => larguraDados_ROM-5)
				 port map (DIN => Saida_ULA, DOUT => REG1_ULA_A, ENABLE => Habilita_A, CLK => CLK, RST => Reset);
	...
	ULA1 : entity work.ULASomaSub  generic map(larguraDados => larguraDados_ROM - 5)
				 port map (entradaA => REG1_ULA_A, entradaB => Saida_MUX_ULA, saida => Saida_ULA, seletor => Operacao_ULA, flagZero => zero_signal);
